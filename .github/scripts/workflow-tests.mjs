...
const nativeSteps = ci.jobs["platform-test"].steps;
const formattersIndex = nativeSteps.findIndex(s => s.run === "bash ./provision.sh format-tools");
const nativeTestIndex = nativeSteps.findIndex(s => s.name === "Run native Go tests");
assert.ok(formattersIndex >= 0 && formattersIndex < nativeTestIndex, "provision formatter regression prerequisites before native tests");
assert.ok(nativeSteps.findIndex(s => s.uses?.startsWith("actions/setup-go@")) < formattersIndex);
assert.equal(nativeSteps[formattersIndex].shell, "bash");
assert.equal(nativeSteps[nativeTestIndex].run, "go test -shuffle=on ./...");
