...
**Status: not started.** Review metadata and cover edits in
`internal/api/bookedit.go` and `internal/api/bookedit_test.go`. Trace input
validation, prepare/replace/commit ordering, failure reconciliation, cache/ETag
updates, and profile/reference ownership. Check that cover-only (fileless) edits
participate in the replacement gate used by cover readers and deletion. Do not
re-review the completed book-response/progress batches or credit dependency reads.
...
3. Replace this task with the next bounded task, leaving it explicitly not started.
   ...
   Do not append a completed-CP33 report; Git records the completed work.
...
5. Stop after CP33 and provide the concise handoff required by `AGENTS.md`.
   CP34 requires explicit continuation in a new thread.
